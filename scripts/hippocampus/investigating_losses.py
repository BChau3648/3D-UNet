...
class GeneralizedDiceLoss(_AbstractDiceLoss):
    ...
    def dice(self, input, target, weight):
        ...
        if input.size(0) == 1:
            print('separates foreground and background voxels')
        ...
        intersect = (input * target).sum(-1)
        intersect_unw = intersect
        intersect = intersect * w_l

        denominator = (input + target).sum(-1)
        denominator_unw = denominator.clamp(min=self.epsilon)
        denominator = (denominator * w_l).clamp(min=self.epsilon)
        print(f"unnormalized: {2*intersect_unw/denominator_unw}")
...
if __name__ == '__main__':
    torch.
    loss = GeneralizedDiceLoss(normalization='softmax')
    input = torch.rand((16, 3, 1, 64, 64))
    input = torch.nn.Softmax(dim=1)(input)
    target = torch.randint(0, 2, (16, 3, 1, 64, 64))

    print(loss.dice(input, target, None))
    print(compute_per_channel_dice(input, target))
